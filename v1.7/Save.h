...
void SaveBack()//悔棋
{
	...
	{  
		...
		for(i=11;i<89;i++)fscanf(fp, "%d %d ", &file->Has[i],&file->Grey[i]); 
		file->next=NULL;//生成新结点 
		p->next=file;//插到尾部 
		p_=p;
		p=file;//p后移 
		sprintf(str,"%d %d %d",p->level,p->score,p->pieceleft);
		
	}

	p=p_;
	for(i=11;i<89;i++){Has[i]=p->Has[i];Grey[i]=p->Grey[i];}
	level=p->level;			score=p->score-5;
	pieceleft=p->pieceleft;	NewBlock=p->New;
...
void SaveRead(user *H,FILE *fp)//用于读取list.save文件，因为最常用，其他读取未制作函数
{
	user *p,*file;  
	p=H;
	fscanf(fp, "\n");
...
bool SaveInputPass(user *p,user *input)
{
	...
	while(strcmp(p->pass,input->pass) && --time)
	{
		...
		if(!InputBox(input->pass,15,"请输入【密码】。按取消重新输入昵称。",
			"方块拼图","在这里输入…",0,0,0))
			return 0;
	}
...
void SaveNew(user *input)
{
	...
	if(!InputBox(input->pass, 15, "欢迎新用户，请设置【密码】。不设密码则输入0或不输入。",
		"方块拼图","在这里输入…",0,0,0))
		strcpy(input->pass,"0");//点取消没有设置密码
	if(!strcmp(input->pass,"在这里输入…")||input->pass[0]=='\0')strcpy(input->pass,"0");//没设密码但没有删除默认提示
...
wrong://输入错误5次的起始点
	...
	if(!InputBox(input->name,15, "请输入您的【昵称】，新用户输入后生成新账户。","方块拼图","在这里输入…",0,0,0))
		return 0;
...
bool SaveChange(int type)//type=1改名字2改密码
{
wrong:
	user *input=(user*)malloc(sizeof(user));
	if(!InputBox(input->name,15, "请输入【昵称】。","方块拼图","在这里输入…",0,0,0))
		return 0;
	if(!input->name[0] || !strcmp(input->name,"在这里输入…")){
	...
	{  
		...
		if(!strcmp(p->name,input->name))//如果相同
		{
			...
			{
				find=1;
				if(!InputBox(input->name,15, "请输入【新昵称】。","方块拼图","在这里输入…",0,0,0))
					return 0;
				if(!input->name[0] || !strcmp(input->name,"在这里输入…")){
					ClickMSGSml("错误","昵称不能为空！",1);goto wrong;}
			...
			else if(type==2)
			{
				...
				{
					if(!InputBox(input->pass,15, "请输入新【密码】，不设置直接确定。","方块拼图","在这里输入…",0,0,0))
						return 0;
					if(!strcmp(input->pass,"在这里输入…")||input->pass[0]=='\0')strcpy(input->pass,"0");
					if(!InputBox(secure,15, "请【确认】新【密码】。","方块拼图","在这里输入…",0,0,0))
						return 0;
...
wrong:
	user *input=(user*)malloc(sizeof(user));
	if(!InputBox(input->name,15, "请输入您要新建的【昵称】。","方块拼图","在这里输入…",0,0,0))
		return 0;
	if(!input->name[0] || !strcmp(input->name,"在这里输入…")){
		ClickMSGSml("错误","昵称不能为空！",1);goto wrong;}
...
wrong:
	user *input=(user*)malloc(sizeof(user));
	if(!InputBox(input->name,15, "请输入您要删除的【昵称】。","方块拼图","在这里输入…",0,0,0))
		return 0;
	if(!input->name[0] || !strcmp(input->name,"在这里输入…")){
		ClickMSGSml("错误","昵称不能为空！",1);goto wrong;}
